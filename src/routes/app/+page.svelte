<script lang="ts">
	...
	let totalActive = $derived(
		MOCK_REWARDS.filter((r) => r.status === 'active').reduce((sum, r) => sum + r.amount, 0)
	);
	let totalPaid = $derived(
		MOCK_REWARDS.filter((r) => r.status === 'completed').reduce((sum, r) => sum + r.amount, 0)
	);
</script>
...
{#if !walletStore.isConnected}
	<div
		class="flex h-[70vh] flex-col items-center justify-center text-center px-4"
		use:reveal={{ y: 20 }}
	>
		<div
...
{:else}
	<div class="space-y-12">
		...
		>
			...
			<a
				href="/app/create"
				class="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-crx-orange px-6 text-sm font-medium text-white transition-all hover:bg-crx-orange-hover active:scale-95 shadow-sm"
			>
				<Plus class="h-4 w-4" />
				New Reward
			</a>
		...
		<div class="grid gap-6 md:grid-cols-3" use:reveal={{ y: 20, delay: 0.1 }}>
			...
			<div class="rounded-2xl border border-crx-border-subtle bg-white p-6 shadow-sm">
				...
				<div class="mt-4 text-3xl font-bold text-crx-black">2</div>
			</div>
		</div>
